...
save_mfa() {
  aws configure set aws_access_key_id $AWS_ACCESS_KEY_ID --profile sb-terraform-mfa-session
  aws configure set aws_secret_access_key $AWS_SECRET_ACCESS_KEY --profile sb-terraform-mfa-session
  aws configure set aws_session_token $AWS_SESSION_TOKEN --profile sb-terraform-mfa-session
}
